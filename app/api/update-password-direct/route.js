// app/api/update-password-direct/route.js
...
export async function POST(req) {
  try {
    ...
    // Verificar que el usuario tenga permisos para cambiar contraseña directamente
    if (!session?.user?.id || !session?.user?.forcePasswordChange) {
      return NextResponse.json(
        { error: 'No autorizado para este tipo de cambio de contraseña' },
        { status: 403 }
      );
    }
    ...
    console.log(`✅ Contraseña cambiada exitosamente para usuario (cambio directo): ${userId}`);
    ...
    console.error('❌ Error al cambiar contraseña directamente:', error);
    return NextResponse.json(
